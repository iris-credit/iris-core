// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity ^0.8.0;

import "../helpers/PeripheryUnitTest.sol";

import {ERC20Mock} from "../../unit/helpers/mocks/ERC20Mock.sol";

contract PermitAdapterUnitTest is PeripheryUnitTest {
    ERC20Mock internal permitToken;

    function setUp() public override {
        super.setUp();

        permitToken = new ERC20Mock("Permit Token", "PT", 18);
    }

    function testPermit(uint256 amount, uint256 privateKey, address spender, uint256 deadline) public {
        vm.assume(spender != address(0));
        // Solady pins the canonical Permit2's allowance at infinity and rejects finite permits for it.
        vm.assume(spender != PermitUtils.PERMIT2);
        amount = bound(amount, MIN_TEST_AMOUNT, MAX_TEST_AMOUNT);
        deadline = bound(deadline, block.timestamp, type(uint48).max);
        privateKey = bound(privateKey, 1, type(uint160).max);

        address user = vm.addr(privateKey);

        bundle.push(_permit(address(permitToken), privateKey, spender, amount, deadline, false));
        bundle.push(_permit(address(permitToken), privateKey, spender, amount, deadline, true));

        vm.prank(user);
        bundler3.multicall(bundle);

        assertEq(permitToken.allowance(user, spender), amount, "allowance(user, spender)");
    }

    function testPermitRevert(uint256 amount, uint256 privateKey, address spender, uint256 deadline) public {
        vm.assume(spender != address(0));
        // Solady pins the canonical Permit2's allowance at infinity and rejects finite permits for it.
        vm.assume(spender != PermitUtils.PERMIT2);
        amount = bound(amount, MIN_TEST_AMOUNT, MAX_TEST_AMOUNT);
        deadline = bound(deadline, block.timestamp, type(uint48).max);
        privateKey = bound(privateKey, 1, type(uint160).max);

        address user = vm.addr(privateKey);

        bundle.push(_permit(address(permitToken), privateKey, spender, amount, deadline, false));
        bundle.push(_permit(address(permitToken), privateKey, spender, amount, deadline, false));

        vm.prank(user);
        vm.expectRevert(ERC20.InvalidPermit.selector);
        bundler3.multicall(bundle);
    }

    function testTransferFrom(uint256 amount, uint256 privateKey, uint256 deadline) public {
        amount = bound(amount, MIN_TEST_AMOUNT, MAX_TEST_AMOUNT);
        deadline = bound(deadline, block.timestamp, type(uint48).max);
        privateKey = bound(privateKey, 1, type(uint160).max);

        address user = vm.addr(privateKey);

        bundle.push(_permit(address(permitToken), privateKey, address(generalAdapter1), amount, deadline, false));
        bundle.push(_erc20TransferFrom(address(permitToken), amount));

        deal(address(permitToken), user, amount);

        vm.prank(user);
        bundler3.multicall(bundle);

        assertEq(permitToken.balanceOf(address(generalAdapter1)), amount, "balanceOf(generalAdapter1)");
        assertEq(permitToken.balanceOf(user), 0, "balanceOf(user)");
    }
}
