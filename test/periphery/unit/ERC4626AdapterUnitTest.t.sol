...
contract ERC4626AdapterUnitTest is PeripheryUnitTest {
    ...
    function testErc4626MintSlippageExceeded(uint256 shares) public {
        ...
        bundle.push(_erc20TransferFrom(debtToken, assets * 2));
        bundle.push(_erc4626Mint(address(vault), shares, assets.rDivDown(shares), USER));

        deal(debtToken, address(vault), 1);

        deal(debtToken, USER, assets * 2);
    ...
    function testErc4626Mint(uint256 shares) public {
        ...
        bundle.push(_erc20TransferFrom(debtToken, assets));
        bundle.push(_erc4626Mint(address(vault), shares, assets.rDivDown(shares), USER));

        deal(debtToken, USER, assets);

        vm.prank(USER);
        bundler3.multicall(bundle);

        assertEq(ERC20(debtToken).balanceOf(address(vault)), assets, "debt.balanceOf(vault)");
        assertEq(ERC20(debtToken).balanceOf(address(generalAdapter1)), 0, "debt.balanceOf(generalAdapter1)");
        assertEq(vault.balanceOf(address(generalAdapter1)), 0, "vault.balanceOf(generalAdapter1)");
        assertEq(vault.balanceOf(USER), shares, "vault.balanceOf(USER)");

        assertEq(
            ERC20(debtToken).allowance(address(generalAdapter1), address(vault)),
            0,
            "debtToken.allowance(generalAdapter1, vault)"
        );
    }
    ...
    function testErc4626WithdrawSlippageExceeded(uint256 deposited, uint256 assets) public {
        deposited = bound(deposited, MIN_TEST_AMOUNT + 1, MAX_TEST_AMOUNT);

        _depositVault(deposited);

        // Don't withdraw max to avoid being limited by `maxWithdraw`.
        assets = bound(assets, MIN_TEST_AMOUNT, deposited - 1);
        ...
        deal(debtToken, address(vault), deposited - 1);
    ...
    function testErc4626Withdraw(uint256 deposited, uint256 assets) public {
        deposited = bound(deposited, MIN_TEST_AMOUNT, MAX_TEST_AMOUNT);

        uint256 minted = _depositVault(deposited);

        assets = bound(assets, MIN_TEST_AMOUNT, deposited);
        ...
        assertEq(ERC20(debtToken).balanceOf(address(vault)), deposited - assets, "debt.balanceOf(vault)");
        assertEq(ERC20(debtToken).balanceOf(address(generalAdapter1)), 0, "debt.balanceOf(generalAdapter1)");
        assertEq(ERC20(debtToken).balanceOf(RECEIVER), assets, "debt.balanceOf(RECEIVER)");
    ...
    function testErc4626Redeem(uint256 deposited, uint256 shares) public {
        ...
        assertEq(ERC20(debtToken).balanceOf(address(vault)), deposited - withdrawn, "debt.balanceOf(vault)");
        assertEq(ERC20(debtToken).balanceOf(address(generalAdapter1)), 0, "debt.balanceOf(generalAdapter1)");
        assertEq(ERC20(debtToken).balanceOf(RECEIVER), withdrawn, "debt.balanceOf(RECEIVER)");
