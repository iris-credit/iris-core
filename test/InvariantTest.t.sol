...
abstract contract InvariantTest is ForkTest {
    ...
    function _maxWithdrawCollateral(address pod) internal view returns (uint256) {
        ...
        (,, uint256 fixedLeg, uint256 floatingLeg,) = iris.accrueLegsView(pod);
        uint256 badBond = floatingLeg.zeroFloorSub(fixedLeg).zeroFloorSub(pos.bond);
        ...
        uint256 irisMin =
            (uint256(pos.debt) + fixedLeg + residual + badBond).mulDivUp(WAD, lltv).mulDivUp(ORACLE_PRICE_SCALE, price);
